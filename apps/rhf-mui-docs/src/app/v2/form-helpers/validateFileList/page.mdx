...
This function utilized by [RHFFileUploader](/v2/components/mui/RHFFileUploader)
ensures only files meeting the required criteria are passed for further processing.
...
2. `accept`: An optional string that specifies the allowed file types or extensions,
  following the standard **input[type="file"]** `accept` attribute format (e.g., `.png, .jpg, image/*`).
  Files not matching the criteria will be rejected.

3. `maxSize`: An optional numeric value (in bytes) that sets the maximum allowed
  file size. Files exceeding this limit will be rejected.

4. `maxFiles`: Maximum number of files allowed for upload. Any additional files,
  even if valid, will be added to the `rejectedFiles` array and excluded from
  the accepted files.
